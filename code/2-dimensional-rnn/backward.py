...
for t in reversed(range(time_steps - 1)):
    dL_dh[t] = (dL_dh[t+1] @ W_h.T + errors[t] @ W_y) * \
        tanh_prime(pre_activations[t])

# Initialize multiple perturbation vectors
num_directions = hidden_size
delta_h_t = np.eye(hidden_size)

# Store the log sum of Lyapunov exponents
...
record_log(ans["deltas"], 2, os.path.join(os.path.dirname(__file__), "backward.txt"))
